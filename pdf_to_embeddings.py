...
class PDFToEmbeddingsConverter:
    ...
    def extract_text_from_pdf(self, pdf_path: str) -> str:
        ...
        try:
            with open(pdf_path, "rb") as file:
                ...
                logger.info(
                    f"Successfully extracted text from {pdf_path} ({len(pdf_reader.pages)} pages)"
                )
                return text.strip()
    ...
    def generate_embeddings(
        self, text_chunks: List[str], model: str = "text-embedding-ada-002"
    ) -> List[List[float]]:
        """
        ...
        try:
            ...
            for i, chunk in enumerate(text_chunks):
                ...
                if (i + 1) % 10 == 0:
                    logger.info(
                        f"Generated embeddings for {i + 1}/{len(text_chunks)} chunks"
                    )

            logger.info(
                f"Successfully generated embeddings for {len(text_chunks)} chunks"
            )
            return embeddings
    ...
    def upload_to_pinecone(
        ...
        if not self.pinecone_api_key:
            logger.warning(
                "Pinecone API key not provided. Skipping upload to Pinecone."
            )
            return
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded {len(vectors)} vectors to Pinecone index '{index_name}'"
            )
...
def main():
    ...
    try:
        ...
        for pdf_file in pdf_files:
            ...
            logger.info(
                f"Generated {results['total_chunks']} chunks with {results['embedding_dimension']}-dimensional embeddings"
            )

    ...
    def rollback_migration(self, migration_name: str) -> bool:
        ...
        try:
                ...
                if os.path.exists(migration_path):
                    ...
                    if 'def rollback(' in migration_code:
                        ...
                        if 'rollback' in locals():
                            ...
                            logger.info(f"✅ Rollback function executed for {migration_name}")
                        else:
                            logger.warning(f"⚠️ Rollback function not found in {migration_name}")
                    else:
                        logger.warning(f"⚠️ No rollback function found in {migration_name}")
            ...
            cursor.execute("""
                ...
                WHERE migration_name = %s AND environment = %s
            """, (json.dumps(datetime.now().isoformat()), migration_name, self.environment))
...
def main():
    ...
    parser.add_argument('--database-url', '-d', required=True,
                       help='Database connection URL')
    parser.add_argument('--dry-run', action='store_true',
                       help='Perform dry run without executing migrations')
    parser.add_argument('--rollback', '-r', metavar='MIGRATION',

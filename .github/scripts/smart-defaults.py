...
class SmartDefaults:
            ...
            if result.returncode == 0:
                ...
                if result.returncode == 0 and "ACTIVE" in result.stdout:
                    ...
                    if gcp_info["project_id"]:
                        result = subprocess.run(['gcloud', 'projects', 'describe', gcp_info["project_id"]], 
                                             capture_output=True, text=True)
                        if result.returncode == 0:
                            gcp_info["project_exists"] = True
                            
                            # Check for Artifact Registry
                            result = subprocess.run(['gcloud', 'artifacts', 'repositories', 'list'], 
                                                 capture_output=True, text=True)
                            if result.returncode == 0:
                                repos = result.stdout.strip().split('\n')[1:]  # Skip header
                                if repos:
                                    gcp_info["artifacts_repo"] = repos[0].split()[0]  # First repo name
    ...
    def _analyze_secret_patterns(self) -> Dict[str, Any]:
        ...
        secret_patterns = {
            ...
            "pinecone": {
                "patterns": ["PINECONE_API_KEY", "PINECONE_ENVIRONMENT", "PINECONE_INDEX_NAME"],
    ...
    def generate_suggestions(self) -> Dict[str, Any]:
        ...
        if env_info["gcp"]["gcloud_available"]:
            if env_info["gcp"]["project_id"]:
                suggestions["automatic"]["GCP_PROJECT_ID"] = env_info["gcp"]["project_id"]
                suggestions["smart_defaults"]["GCP_PROJECT_ID"] = f"✅ Auto-detected: {env_info['gcp']['project_id']}"
            
            if env_info["gcp"]["region"]:
                suggestions["automatic"]["REGION"] = env_info["gcp"]["region"]
                suggestions["smart_defaults"]["REGION"] = f"✅ Auto-detected: {env_info['gcp']['region']}"
            
            if not env_info["gcp"]["authenticated"]:
                suggestions["next_steps"].append("🔐 Run: gcloud auth login")
            
            if not env_info["gcp"]["project_exists"]:
                suggestions["next_steps"].append("☁️ Create GCP project or set correct project")
        
        # GitHub suggestions
        if env_info["github"]["repository"]:
            suggestions["automatic"]["SERVICE_NAME"] = env_info["github"]["repository"]
            suggestions["smart_defaults"]["SERVICE_NAME"] = f"✅ Auto-detected: {env_info['github']['repository']}"
        
        # Local environment suggestions
        if env_info["local"]["env_files"]:
            suggestions["next_steps"].append(f"📁 Found environment files: {', '.join(env_info['local']['env_files'])}")
            suggestions["next_steps"].append("💡 Check these files for existing secret values")
        
        # Secret pattern suggestions
        for category, patterns in env_info["secrets"].items():
            if patterns:
                suggestions["manual"][category] = patterns
                suggestions["next_steps"].append(f"🔍 Found {category} patterns in code - configure these secrets")
...
"""
    ...
    def print_analysis(self):
        ...
        print(f"\n🔍 Environment Detection:")
        print(f"   GCP: {'✅ Available' if env_info['gcp']['gcloud_available'] else '❌ Not available'}")
        print(f"   GitHub: {'✅ Connected' if env_info['github']['repository'] else '❌ Not connected'}")
        print(f"   Docker: {'✅ Available' if env_info['local']['docker_available'] else '❌ Not available'}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Smart Defaults & Environment Detection')
    parser.add_argument('--project-root', '-p', default='.', help='Project root directory')
    parser.add_argument('--output', '-o', default='smart-defaults-analysis.json', help='Output file')
    parser.add_argument('--script', '-s', default='setup-cicd.sh', help='Setup script file')

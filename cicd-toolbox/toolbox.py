...
class IntelligentCICDToolbox:
    ...
    def log_execution(self, step: str, message: str, status: str = "info"):
        ...
        # Also log to console
        status_emoji = {"success": "✅", "error": "❌", "warning": "⚠️", "info": "ℹ️"}.get(status, "ℹ️")
    ...
    def set_project(self, project_id: str) -> Tuple[bool, str]:
        ...
        self.log_execution("Project Configuration", f"Setting GCP project to: {project_id}", "info")
        ...
        try:
            ...
            self.log_execution("Project Configuration", f"Project set successfully to: {project_id}", "success")
    ...
    def setup_infrastructure(self, project_id: str) -> bool:
        ...
        self.log_execution("Infrastructure Setup", "Starting GCP infrastructure setup...", "info")
    ...
    def _setup_gcp_infrastructure(self, project_id: str) -> bool:
        ...
        self.log_execution("GCP Infrastructure", "Setting up GCP infrastructure...", "info")
        ...
        results = []
        self.log_execution("GCP Infrastructure", f"Enabling {len(apis)} required APIs...", "info")
        ...
        for i, api in enumerate(apis):
            ...
            try:
                ...
                self.log_execution("GCP Infrastructure", f"API enabled: {api}", "success")
            except subprocess.CalledProcessError as e:
                results.append(f"❌ {api}: {e.stderr.strip()}")
                self.log_execution("GCP Infrastructure", f"API failed: {api} - {e.stderr.strip()}", "error")
        ...
        if service_account:
            ...
            self.log_execution("GCP Infrastructure", "Service account creation failed", "error")
            return False
        
        # Setup Workload Identity
        self.log_execution("GCP Infrastructure", "Setting up Workload Identity Federation...", "info")
        wif_provider, wif_results = self._setup_workload_identity(project_id)
        results.extend(wif_results)
        
        # Log final results
        self.log_execution("GCP Infrastructure", "Infrastructure setup completed successfully!", "success")
    ...
    def _setup_workload_identity(self, project_id: str) -> Tuple[str, List[str]]:
        ...
        try:
            subprocess.run([
                ...
                "--description", "Pool for GitHub Actions CI/CD pipeline",
                "--project", project_id
            ], capture_output=True, text=True, check=True)
        ...
        try:
            ...
            if "ALREADY_EXISTS" in e.stderr:
                results.append(f"ℹ️ Workload Identity Provider already exists: {provider_id}")
            else:
                results.append(f"❌ Failed to create provider: {e.stderr.strip()}")
        
        return f"projects/{project_id}/locations/global/workloadIdentityPools/{pool_id}/providers/{provider_id}", results
    ...
    def generate_secrets_template(self, project_id: str) -> Dict[str, str]:
        ...
        self.log_execution("Secrets Generation", "Generating intelligent secrets template...", "info")
        ...
        self.log_execution("Secrets Generation", f"Generated {len(self.secrets_data)} secrets", "success")
    ...
    def push_secrets_to_github(self) -> Tuple[bool, Any]:
        ...
        self.log_execution("GitHub Secrets Push", "Starting to push secrets to GitHub...", "info")
        ...
        try:
            ...
            self.log_execution("GitHub Secrets Push", f"Pushing to repository: {repo_owner}/{repo_name}", "info")
            
            results = []
            failed_secrets = []
            
            for secret_name, secret_value in self.secrets_data.items():
                self.log_execution("GitHub Secrets Push", f"Setting secret: {secret_name}", "info")
                ...
                try:
                    subprocess.run([
                        ...
                        "--body", secret_value
                    ], check=True, input=secret_value.encode())
                    
                    results.append(f"✅ {secret_name}: Set successfully")
                    self.log_execution("GitHub Secrets Push", f"Secret set: {secret_name}", "success")
                except subprocess.CalledProcessError as e:
                    error_msg = f"❌ {secret_name}: Failed to set"
                    results.append(error_msg)
                    failed_secrets.append({"secret": secret_name, "error": str(e)})
                    self.log_execution("GitHub Secrets Push", f"Secret failed: {secret_name}", "error")
            
            if failed_secrets:
                self.log_execution("GitHub Secrets Push", f"Failed to push {len(failed_secrets)} secrets", "warning")
            else:
                self.log_execution("GitHub Secrets Push", "All secrets pushed successfully!", "success")
            
            # Store results
            self.step_results["github_secrets_push"] = {
                "status": "completed" if not failed_secrets else "completed_with_errors",
                "timestamp": datetime.now().isoformat(),
                "total_secrets": len(self.secrets_data),
    ...
    def trigger_pipeline(self, environment: str = "staging", skip_steps: List[str] = None, force_deploy: bool = False) -> Tuple[bool, Any]:
        """Trigger CI/CD pipeline with environment and step control"""
        if not skip_steps:
            skip_steps = []
        
        self.current_step = "Pipeline Trigger"
        self.log_execution("Pipeline Trigger", f"Triggering pipeline for {environment} environment...", "info")
        ...
        try:
            ...
            if current_branch != "main":
                self.log_execution("Pipeline Trigger", f"Switching from {current_branch} to main branch...", "info")
            ...
            with open(".pipeline-trigger", "w") as f:
                ...
                f.write(f"# Skip Steps: {', '.join(skip_steps) if skip_steps else 'None'}\n")
            ...
            self.log_execution("Pipeline Trigger", f"Pipeline triggered successfully for {environment}", "success")
    ...
    def get_execution_summary(self) -> List[str]:
        ...
        for step, result in self.step_results.items():
            ...
            summary.append(f"{status_emoji} {step.replace('_', ' ').title()}: {result['status']}")
    ...
    def get_next_steps(self) -> List[str]:
        ...
        if not self.step_results.get("project_analysis"):
            ...
            next_steps.append("🔧 Setup GCP Infrastructure (APIs, Service Account, WIF)")

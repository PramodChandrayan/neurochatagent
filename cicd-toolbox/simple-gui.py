...
# Page config
st.set_page_config(
    page_title="🚀 CI/CD Deployment Toolbox", page_icon="🚀", layout="wide"
)
    ...
    def trigger_pipeline(self):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                st.info(
                    "Consider committing your changes before triggering the pipeline"
                )
                ...
                if st.button("📝 Commit and Push Changes", key="commit_push"):
                    ...
                    commit_message = st.text_input(
                        ...
                        value="Update deployment configuration",
                        key="commit_msg",
                    )
                    if st.button("✅ Commit", key="commit_btn"):
                        ...
                        if st.button("🚀 Push to Trigger Pipeline", key="push_btn"):
                            ...
                            st.success(
                                "✅ Pipeline triggered! Check GitHub Actions for progress."
                            )
                            return True
            else:
...
def main():
    st.markdown(
        '<div class="main-header"><h1>🚀 CI/CD Deployment Toolbox</h1><p>Automated deployment to GCP Cloud Run</p></div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="step-box"><h3>🔐 Authentication Status</h3></div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="step-box"><h3>🏗️ Step 1: GCP Infrastructure Setup</h3></div>',
        unsafe_allow_html=True,
    )

    if not toolbox.project_id:
        toolbox.project_id = st.text_input(
            "Enter your GCP Project ID:", placeholder="your-project-id"
        )
    ...
    if toolbox.project_id and not toolbox.gcp_setup_done:
        ...
        if toolbox.project_analysis and st.button(
            "⚙️ Setup GCP Infrastructure", type="primary", key="setup_gcp"
        ):
    ...
    if toolbox.gcp_setup_done:
        st.markdown(
            '<div class="step-box"><h3>🔑 Step 2: GitHub Secrets</h3></div>',
            unsafe_allow_html=True,
        )

        if not toolbox.secrets_pushed:
            if st.button(
                "🔑 Prepare GitHub Secrets", type="primary", key="prepare_secrets"
            ):
    ...
    if toolbox.secrets_pushed:
        st.markdown(
            '<div class="step-box"><h3>🚀 Step 3: Trigger Pipeline</h3></div>',
            unsafe_allow_html=True,
        )

        if st.button(
            "🚀 Trigger CI/CD Pipeline", type="primary", key="trigger_pipeline"
        ):
            with st.spinner("Triggering pipeline..."):
                ...
                if success:
                    ...
                    st.warning(
                        "⚠️ Pipeline trigger failed. Check the instructions above."
                    )

    # Final success message
    if all(steps.values()):
        st.markdown(
            '<div class="step-box step-success"><h3>🎉 Deployment Complete!</h3></div>',
            unsafe_allow_html=True,
        )

...
def main():
    st.markdown('<h1 class="main-header">🚀 Intelligent CI/CD Toolbox</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; font-size: 1.2rem; color: #666;">Real-time feedback, automatic secrets pushing, pipeline control, and deployment strategy</p>', unsafe_allow_html=True)
    ...
    analysis = None
    if hasattr(st.session_state, 'project_analysis') and st.session_state.project_analysis:
    ...
    if analysis:
        ...
        with col3:
            st.metric("Cloud Provider", analysis["project_info"]["cloud_provider"] or 'Not detected')
        with col4:
    ...
    if hasattr(st.session_state, 'project_analysis'):
        ...
        with col3:
            ...
            enable_monitoring = st.checkbox(
                "Enable Monitoring",
                value=True,
                help="Enable Cloud Monitoring"
            )
            
            enable_logging = st.checkbox(
                "Enable Logging",
                value=True,
                help="Enable Cloud Logging"
            )
    ...
    if hasattr(st.session_state, 'project_analysis'):
        ...
        with col1:
            if deployment_config:
                environment = st.selectbox(
                    ...
                    index=["staging", "production", "development"].index(deployment_config.get("environment", "staging")),
                    help="Choose the target environment for deployment"
                )
            else:
                environment = st.selectbox(
        ...
        # Collect skip steps
        skip_steps = []
        if skip_tests: skip_steps.append("tests")
        if skip_lint: skip_steps.append("lint")
        if skip_build: skip_steps.append("build")
        if skip_scan: skip_steps.append("security-scan")
        if skip_deploy: skip_steps.append("deploy")
        if skip_notify: skip_steps.append("notify")
        if skip_docs: skip_steps.append("docs")
        if skip_metrics: skip_steps.append("metrics")
        if skip_backup: skip_steps.append("backup")
    ...
    with col1:
        ...
        if gcloud_auth:
            st.success(f"✅ Authenticated as: {gcloud_data[0].get('account', 'Unknown')}")
        else:

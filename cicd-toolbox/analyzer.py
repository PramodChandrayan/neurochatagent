...
class IntelligentProjectAnalyzer:
    ...
    def analyze_project(self) -> Dict[str, Any]:
        ...
        print(f"✅ Detected: {project_info['type']} project with {project_info['framework']} framework")
        ...
        requirements = self._generate_requirements(project_info, dependencies, config_secrets, code_secrets, deployment_configs)
    ...
    def _detect_cloud_provider(self) -> str:
        ...
        # Check for GCP
        if (self.project_path / ".gcloudignore").exists() or (self.project_path / "app.yaml").exists():
            cloud_providers.append("gcp")
        
        # Check for AWS
        if (self.project_path / ".ebignore").exists() or (self.project_path / "elasticbeanstalk").exists():
            cloud_providers.append("aws")
        
        # Check for Azure
        if (self.project_path / ".azure").exists() or (self.project_path / "azure.yaml").exists():
    ...
    def _analyze_deployment_configs(self) -> Dict[str, Any]:
        ...
        if (self.project_path / "docker-compose.yml").exists() or (self.project_path / "docker-compose.yaml").exists():
            deployment_configs["docker_compose"] = True
        
        # Check for Kubernetes
        for k8s_file in self.project_path.rglob("*.yaml"):
            if "kubernetes" in k8s_file.read_text().lower() or "apiVersion" in k8s_file.read_text():
                deployment_configs["kubernetes"] = True
                break
        
        # Check for Terraform
        if (self.project_path / "*.tf").exists() or (self.project_path / "*.tfvars").exists():
            deployment_configs["terraform"] = True
        
        # Check for CloudFormation
        if (self.project_path / "*.template").exists() or (self.project_path / "*.yaml").exists():
    ...
    def _is_system_env_var(self, var_name: str) -> bool:
        ...
        system_vars = {
            ...
            "HOSTNAME", "HOST", "PORT", "NODE_ENV", "PYTHONPATH", "JAVA_HOME",
            "GOPATH", "RUST_BACKTRACE", "RUBY_VERSION"
        }
        return var_name in system_vars
    
    def _generate_requirements(self, project_info: Dict, dependencies: Dict, config_secrets: Set, code_secrets: Set, deployment_configs: Dict) -> Dict[str, Any]:
        ...
        if project_info["type"] == "python":
            if "streamlit" in project_info["framework"]:
                recommendations.append("Consider using Streamlit Cloud for easy deployment")
            if "flask" in project_info["framework"] or "django" in project_info["framework"]:
                recommendations.append("Web framework detected - consider containerization for deployment")
        
        if deployment_configs["docker"]:
            recommendations.append("Docker detected - ready for containerized deployment")
        
        if deployment_configs["kubernetes"]:
            recommendations.append("Kubernetes manifests detected - ready for K8s deployment")
        
        if "OPENAI_API_KEY" in runtime_secrets:
            recommendations.append("OpenAI integration detected - ensure API key is securely managed")
        
        if "PINECONE_API_KEY" in runtime_secrets:
            recommendations.append("Pinecone vector database detected - ensure API credentials are secure")
